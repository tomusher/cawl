...
# Export the same version-matched assets for a fully manual deployment.
...
result with `--inventory`. By default the bundled Compose file uses the image
tag matching the CLI version; set `cawl_server_image_tag` in the configuration
to override it. The CLI and deployment assets are intentionally version-matched;
